...
public class MainActivity extends Activity {
    ...
    private static final String WEBSITE_URL = "https://quickgovph.vercel.app/";
    private static final String PREFS = "app.vercel.quickgovph.prefs";
    ...
    private void setupWebView() {
        ...
        webView.setWebViewClient(new WebViewClient() {
            @Override public void onPageFinished(WebView view, String pageUrl) {
                view.evaluateJavascript("javascript:(function() { var style = document.createElement('style'); style.innerHTML = '* { -webkit-user-select: none !important; user-select: none !important; }'; document.head.appendChild(style); })();", null);
        ...
        webView.setWebChromeClient(new WebChromeClient() {
            ...
            @Override public boolean onShowFileChooser(WebView v, ValueCallback<Uri[]> cb, FileChooserParams params) {
                ...
                try{startActivityForResult(intent,1001);}catch(Exception e){filePathCallback=null;return false;}
                return true;
            }
        });
    }
    ...
    private static final int PERM_REQUEST_CODE = 1002;
    private void promptForAdvancedPermissions() {
        if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.M) {
            loadWebsite(); return;
        }
        java.util.List<String> needed = new java.util.ArrayList<>();
        if (checkSelfPermission("android.permission.ACCESS_FINE_LOCATION") != android.content.pm.PackageManager.PERMISSION_GRANTED) needed.add("android.permission.ACCESS_FINE_LOCATION");
        if (checkSelfPermission("android.permission.ACCESS_COARSE_LOCATION") != android.content.pm.PackageManager.PERMISSION_GRANTED) needed.add("android.permission.ACCESS_COARSE_LOCATION");
        if (checkSelfPermission("android.permission.CAMERA") != android.content.pm.PackageManager.PERMISSION_GRANTED) needed.add("android.permission.CAMERA");
        if (checkSelfPermission("android.permission.RECORD_AUDIO") != android.content.pm.PackageManager.PERMISSION_GRANTED) needed.add("android.permission.RECORD_AUDIO");
        if (checkSelfPermission("android.permission.GET_ACCOUNTS") != android.content.pm.PackageManager.PERMISSION_GRANTED) needed.add("android.permission.GET_ACCOUNTS");
        if (!needed.isEmpty()) {
    ...
    @Override protected void onActivityResult(int req,int res,android.content.Intent data){if(req==1001&&filePathCallback!=null){filePathCallback.onReceiveValue(WebChromeClient.FileChooserParams.parseResult(res,data));filePathCallback=null;}}
